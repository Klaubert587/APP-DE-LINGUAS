...
// --- FUNÇÕES PARA O DIÁLOGO SIMULADO ---

function carregarDialogoSelecionado() {
    const select = document.getElementById('select-dialogo-simulado');
    const nomePasta = select.value;
    
    if (nomePasta && bancoDeDados[nomePasta]) {
        // Define a pasta e volta para a primeira frase
        pastaAtual = nomePasta;
        seqAtual = 0; 

        const exercicios = bancoDeDados[pastaAtual];
        const displayPrincipal = document.getElementById('pergunta-app-simulado');
        
        if (displayPrincipal) {
            const ex = exercicios[seqAtual];
            displayPrincipal.innerText = ex.coreano;
            
            // ESSENCIAL: Sincroniza a memória para tradução e resposta
            respostaCoreanaCorreta = ex.coreano;
            ...
            document.getElementById('escrita-usuario-simulado').value = "";
        }
    }
}

function proximoPassoSimulado() {
    if (!pastaAtual || !bancoDeDados[pastaAtual]) return;

    const exercicios = bancoDeDados[pastaAtual];
    seqAtual++;

    // Se chegar ao fim das frases da pasta, volta pro começo
    if (seqAtual >= exercicios.length) {
        seqAtual = 0;
        alert("Fim do diálogo! Reiniciando...");
    }

    const novoEx = exercicios[seqAtual];
    document.getElementById('pergunta-app-simulado').innerText = novoEx.coreano;
    
    // Atualiza a memória de tradução para a nova frase
    respostaCoreanaCorreta = novoEx.coreano;
    respostaPortuguesCorreta = novoEx.portugues;

    // Limpa os campos para a nova tentativa
    document.getElementById('escrita-usuario-simulado').value = "";
    document.getElementById('area-dica-simulada').innerText = "";
}

function verificarRespostaSimulado() {
    const respostaUsuario = document.getElementById('escrita-usuario-simulado').value;
    
    // Função para ignorar pontos, vírgulas e espaços bobos
    const limpar = (t) => t.toLowerCase().replace(/[.,!?;]/g, "").trim();

    if (limpar(respostaUsuario) === limpar(respostaCoreanaCorreta)) {
        document.getElementById('area-dica-simulada').innerHTML = 
            "<span style='color: #28a745; font-weight: bold;'>✔ CORRETO!</span>";
    } else {
        document.getElementById('area-dica-simulada').innerHTML = 
